...
public class ParticipantManagementServiceTests
{
    ...
    public async Task KickParticipantAsync_ShouldSetStatusToKicked_WhenRequesterIsHost()
    {
        ...
        participant.Status.Should().Be(nameof("KICKED"));
    ...
    public async Task LeaveRoomAsync_ShouldSetStatusToLeft_WhenParticipantLeaves()
    {
        ...
        result.IsSuccess.Should().BeTrue();
        participant.Status.Should().Be(nameof("LEFT"));
        _participantRepositoryMock.Verify(repo => repo.Update(participant), Times.Once);
        _unitOfWorkMock.Verify(uow => uow.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }
    [Fact]
    public async Task GetParticipantsAsync_ShouldFilterAndSortParticipants()
    {
        ...
        {
            new TranslationRoomParticipant { Id = Guid.NewGuid(), TranslationRoomId = roomId, DisplayName = "Alice", Status = nameof("CONNECTED"), Role = TranslationRoomParticipantRole.PARTICIPANT.ToString(), JoinedAt = DateTime.UtcNow.AddMinutes(-5), ListenLanguage = "en", SpeakLanguage = "en" },
            new TranslationRoomParticipant { Id = Guid.NewGuid(), TranslationRoomId = roomId, DisplayName = "Bob", Status = nameof("LEFT"), Role = TranslationRoomParticipantRole.PARTICIPANT.ToString(), JoinedAt = DateTime.UtcNow.AddMinutes(-10), ListenLanguage = "vi", SpeakLanguage = "vi" },
            new TranslationRoomParticipant { Id = Guid.NewGuid(), TranslationRoomId = roomId, DisplayName = "Charlie", Status = nameof("CONNECTED"), Role = TranslationRoomParticipantRole.PARTICIPANT.ToString(), JoinedAt = DateTime.UtcNow.AddMinutes(-2), ListenLanguage = "fr", SpeakLanguage = "fr" }
        };
