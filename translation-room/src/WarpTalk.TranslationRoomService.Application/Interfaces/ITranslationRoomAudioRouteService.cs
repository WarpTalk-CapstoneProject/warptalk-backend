...
public interface ITranslationRoomAudioRouteService
{
    ...
    /// and nobody else (not the listener, not the host) can consent on their behalf. Turning it ON
    /// additionally requires the speaker's standing consent record, exactly as
    /// <see cref="SetVoiceCloneConsentAsync"/> does; turning it OFF never does.
    /// </summary>
    Task<Result<TranslationRoomAudioRouteDto>> ToggleVoiceCloneAsync(Guid roomId, Guid routeId, Guid callerUserId, ToggleVoiceCloneDto dto, CancellationToken ct = default);
    ...
    /// ToggleVoiceCloneAsync (one route at a time, still speaker-only),
