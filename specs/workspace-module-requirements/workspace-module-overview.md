...
| FR-WS-013 | Enterprise | Email ngoài verified domain chỉ được mời khi AllowExternalCollaboration=true và bị ép role External Member. | WT-157; code: WorkspaceInvitationService external policy checks, UpdateWorkspaceSettingsAsync |
...
| FR-WS-018 | Document | Tài liệu bị archive/delete không được dùng làm context cho AI/RAG. | WT-158/DB-AI; code: DeleteDocumentAsync, RedisDocumentEventPublisher delete/archive events |
...
| FR-WS-021 | AI | Document ingestion phát sự kiện qua Redis Stream kết hợp RabbitMQ, phân loại sensitive/AI eligibility và audit kết quả. | WT-158-AI/PII/RabbitMQ; code: IWorkspaceDocumentEventPublisher, RedisDocumentEventPublisher, DocumentAiIngestionConsumerService; target: RabbitMQ durable delivery |
