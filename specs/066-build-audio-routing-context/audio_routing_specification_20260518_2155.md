...
stateDiagram-v2
    ...
    PENDING --> READY : config_ready

    READY --> ENDING : session_ends / system_disabled
    READY --> BROADCASTING : session_starts
    ...
    BROADCASTING --> SPEECH_DELAYED          : stt_latency_high
    BROADCASTING --> TRANSLATION_DELAYED  : translation_latency_high
    BROADCASTING --> VOICE_DELAYED          : tts_latency_high
    BROADCASTING --> STANDARD_VOICE  : voice_clone_unavailable\n/ token_exhausted
    BROADCASTING --> CAPTION_ONLY        : audio_unavailable

    SPEECH_DELAYED --> BROADCASTING          : stt_recovered
    SPEECH_DELAYED --> STANDARD_VOICE          : voice_clone_unavailable\n/ token_exhausted
    SPEECH_DELAYED --> CAPTION_ONLY                : audio_unavailable

    TRANSLATION_DELAYED --> BROADCASTING  : translation_recovered
    TRANSLATION_DELAYED --> STANDARD_VOICE  : voice_clone_unavailable\n/ token_exhausted
    TRANSLATION_DELAYED --> CAPTION_ONLY        : audio_unavailable

    VOICE_DELAYED --> BROADCASTING          : tts_recovered
    VOICE_DELAYED --> STANDARD_VOICE          : voice_clone_unavailable\n/ token_exhausted
    VOICE_DELAYED --> CAPTION_ONLY                : tts_unavailable\n/ audio_unavailable

    STANDARD_VOICE --> BROADCASTING  : voice_clone_recovered\n/ token_recovered
    STANDARD_VOICE --> CAPTION_ONLY        : tts_unavailable\n/ audio_unavailable

    CAPTION_ONLY --> BROADCASTING        : audio_recovered

    %% ─── Pause / Resume (Từ bất kỳ Streaming State nào) ───

    BROADCASTING  --> PAUSED : room_pause
    SPEECH_DELAYED          --> PAUSED : room_pause
    TRANSLATION_DELAYED  --> PAUSED : room_pause
    VOICE_DELAYED          --> PAUSED : room_pause
    STANDARD_VOICE  --> PAUSED : room_pause
    CAPTION_ONLY        --> PAUSED : room_pause

    PAUSED --> BROADCASTING  : room_resume

    %% ─── Termination (Từ các trạng thái không kết thúc) ───

    BROADCASTING  --> ENDING : session_ends / system_disabled
    PAUSED  --> ENDING : session_ends / system_disabled
    SPEECH_DELAYED          --> ENDING : session_ends / system_disabled
    TRANSLATION_DELAYED  --> ENDING : session_ends / system_disabled
    VOICE_DELAYED          --> ENDING : session_ends / system_disabled
    STANDARD_VOICE  --> ENDING : session_ends / system_disabled
    CAPTION_ONLY        --> ENDING : session_ends / system_disabled

    ENDING --> SAVING_OUTPUTS : flush_runtime

    %% ─── Hậu kỳ (Finalization) ───

    SAVING_OUTPUTS --> COMPLETED                 : outputs_linked
    SAVING_OUTPUTS --> SAVE_FAILED : finalization_failed
    SAVING_OUTPUTS --> COMPLETED                 : finalization_abandoned

    SAVE_FAILED --> SAVING_OUTPUTS : flush_runtime (recovery sweep)
    SAVE_FAILED --> COMPLETED            : finalization_abandoned
...
*   **PostgreSQL:** Bản ghi route được tạo với giá trị trạng thái là `PENDING`.
...
*   **PostgreSQL:** Trạng thái route chuyển sang `READY`.
...
*   **PostgreSQL:** Trạng thái canonical lưu là `BROADCASTING`.
...
*   **PostgreSQL:** Trạng thái được đặt thành `PAUSED`.
...
*   **PostgreSQL:** Canonical state chuyển thành `SPEECH_DELAYED`.
...
*   **PostgreSQL:** Trạng thái cập nhật thành `VOICE_DELAYED`.
...
*   **PostgreSQL:** Trạng thái chuyển dịch sang `STANDARD_VOICE`.
...
*   **PostgreSQL:** Trạng thái lưu thành `CAPTION_ONLY`.
...
*   **PostgreSQL:** Trạng thái ghi nhận `ENDING`.
...
*   **PostgreSQL:** Trạng thái hiển thị `SAVING_OUTPUTS`.
...
*   **PostgreSQL:** Đặt trạng thái `SAVE_FAILED`.
...
|---|---|---|---|
| `config_ready` | `PENDING` | `READY` | Hệ thống xác thực xong ngôn ngữ các thành viên. |
| `session_starts` | `READY` | `BROADCASTING` | Host bấm nút bắt đầu phòng họp. |
| `room_pause` | Bất kỳ Streaming State nào | `PAUSED` | Host bấm nút tạm dừng cuộc họp. |
| `room_resume` | `PAUSED` | `BROADCASTING` | Host bấm nút tiếp tục cuộc họp. |
| `stt_latency_high` | `BROADCASTING` | `SPEECH_DELAYED` | Chỉ số trễ trung bình Whisper EMA vượt quá 3000ms. |
| `stt_recovered` | `SPEECH_DELAYED` | `BROADCASTING` | Chỉ số trễ Whisper khôi phục dưới ngưỡng trễ an toàn. |
| `translation_latency_high` | `BROADCASTING` | `TRANSLATION_DELAYED` | Độ trễ NMT dịch máy vượt quá 2500ms. |
| `translation_recovered` | `TRANSLATION_DELAYED` | `BROADCASTING` | Độ trễ dịch máy khôi phục về mức bình thường. |
| `tts_latency_high` | `BROADCASTING` | `VOICE_DELAYED` | Trễ tổng hợp giọng nói của XTTS v2 vượt quá 6000ms. |
| `tts_recovered` | `VOICE_DELAYED` | `BROADCASTING` | Độ trễ tổng hợp âm TTS khôi phục dưới ngưỡng. |
| `voice_clone_unavailable` | `BROADCASTING` / Các delayed state | `STANDARD_VOICE` | Máy chủ XTTS bị sập vật lý hoặc crash trong lúc dịch âm. |
| `token_exhausted` | `BROADCASTING` / Các delayed state | `STANDARD_VOICE` | Người dùng sử dụng hết hạn mức nhân bản được phân bổ. |
| `voice_clone_recovered` | `STANDARD_VOICE` | `BROADCASTING` | Dịch vụ XTTS kết nối lại thành công. |
| `token_recovered` | `STANDARD_VOICE` | `BROADCASTING` | Tài khoản được sạc lại/gia hạn hạn mức token. |
| `tts_unavailable` | `VOICE_DELAYED` / `VOICE_CLONE` | `CAPTION_ONLY` | Sập toàn bộ dịch vụ TTS trung gian, không thể phát giọng. |
| `audio_unavailable` | Bất kỳ Streaming State nào | `CAPTION_ONLY` | Mất kết nối WebRTC Media Gateway, không truyền được âm. |
| `audio_recovered` | `CAPTION_ONLY` | `BROADCASTING` | Khôi phục lại kết nối âm thanh và thiết bị phát đầu ra. |
| `session_ends` | Bất kỳ non-terminal state nào | `ENDING` | Host chủ động bấm nút Kết thúc phòng họp. |
| `system_disabled` | Bất kỳ non-terminal state nào | `ENDING` | Quản trị viên hệ thống cưỡng bức tắt room hoặc sập nguồn. |
| `flush_runtime` | `ENDING` / `SAVE_FAILED` | `SAVING_OUTPUTS` | Kênh truyền hoàn tất đóng, đẩy luồng ghi tệp tự động. |
| `outputs_linked` | `SAVING_OUTPUTS` | `COMPLETED` | Bản dịch, file ghi âm được lưu trữ và liên kết DB thành công. |
| `finalization_failed` | `SAVING_OUTPUTS` | `SAVE_FAILED`| Lỗi mạng tạm thời, lỗi DB ghi tệp trong quá trình hậu kỳ. |
| `finalization_abandoned` | `SAVING_OUTPUTS` / `SAVE_FAILED` | `COMPLETED` | Quá giới hạn quét tự động (5 lần) hoặc lỗi DB nghiêm trọng. |
...
$$\text{CAPTION\_ONLY} > \text{STANDARD\_VOICE} > \text{VOICE\_DELAYED} > \text{TRANSLATION\_DELAYED} > \text{SPEECH\_DELAYED} > \text{BROADCASTING}$$
...
*   **Bài toán:** Khi phòng bị tạm dừng (`PAUSED`), các gói tin telemetry cũ trôi nổi trên mạng hoặc gói tin rác vẫn có thể gửi lên Redis, nếu không có cơ chế chặn, nó sẽ kích hoạt việc cập nhật trạng thái làm phòng bị chuyển từ `PAUSED` ngược lại `DEGRADED` một cách sai lệch.
*   **Giải pháp:** Khi trạng thái ghi nhận là `PAUSED`, toàn bộ tiến trình ghi trạng thái từ Telemetry Sweep vào PostgreSQL bị **chặn hoàn toàn**. Chỉ khi sự kiện `room_resume` được phát ra, trạng thái được Backend chuyển hướng an toàn về lại `BROADCASTING` thì hàng rào bảo vệ mới được dỡ bỏ.
...
*   **Bước 1:** Phòng họp được tạo và lên cấu hình đầy đủ $\rightarrow$ Hệ thống thiết lập trạng thái route là `PENDING`.
*   **Bước 2:** Bộ kiểm tra ngôn ngữ hoàn tất việc xác thực $\rightarrow$ Phát sự kiện `config_ready` $\rightarrow$ Trạng thái chuyển sang `READY`.
*   **Bước 3:** Host nhấn bắt đầu phiên họp $\rightarrow$ Sự kiện `session_starts` được kích hoạt $\rightarrow$ Chuyển sang `BROADCASTING`. Hệ thống bắt đầu truyền âm thanh thời gian thực.
*   **Bước 4:** Phiên họp diễn ra trơn tru. Khi kết thúc, Host nhấn nút kết thúc phòng họp $\rightarrow$ Kích hoạt sự kiện `session_ends` $\rightarrow$ Chuyển sang `ENDING` để đóng băng luồng dữ liệu.
...
*   **Bước 1:** Phòng đang ở trạng thái `BROADCASTING`.
*   **Bước 2:** Hệ thống dịch máy (NMT) đột ngột bị nghẽn mạng, trễ vượt quá 2500ms $\rightarrow$ Telemetry phát hiện và gửi cờ $\rightarrow$ Hệ thống chuyển trạng thái sang `TRANSLATION_DELAYED`.
*   **Bước 3:** Tiếp đó, máy chủ STT cũng bị quá tải, trễ vượt quá 3000ms $\rightarrow$ Cờ `stt_degraded = true` được bật.
*   **Bước 4:** Áp dụng công thức ưu tiên của `AudioRoutePriorityResolver`: Trễ dịch thuật có mức độ ưu tiên ảnh hưởng cao hơn trễ STT, do đó trạng thái phòng họp vẫn được giữ vững ở mức cảnh báo là `TRANSLATION_DELAYED` thay vì bị nhảy loạn.
*   **Bước 5:** Dịch vụ mạng phục hồi, độ trễ dịch máy giảm xuống $\rightarrow$ Telemetry gỡ bỏ cờ trễ dịch thuật $\rightarrow$ Lúc này, cờ trễ STT vẫn còn, hệ thống tự động chuyển đổi an toàn sang trạng thái `SPEECH_DELAYED`.
*   **Bước 6:** STT khôi phục hoàn toàn $\rightarrow$ Tất cả các cờ lỗi được xóa $\rightarrow$ Hệ thống tự động phục hồi luồng âm thanh về trạng thái hoàn hảo `BROADCASTING`.

### Kịch Bản 3: Sự Cố Trực Tiếp Và Hạ Cấp Phụ Đề Subtitle (Text-Only Mode)
*   **Bước 1:** Phòng đang ở trạng thái hoạt động `BROADCASTING`.
...
*   **Bước 3:** Backend C# tiêu thụ sự kiện và lập tức chuyển trạng thái sang `CAPTION_ONLY`.
...
*   **Bước 5:** Sự cố WebRTC được khắc phục $\rightarrow$ AI Worker ghi nhận đường truyền âm thanh hoạt động lại $\rightarrow$ Bắn sự kiện `audio_recovered` $\rightarrow$ Hệ thống chuyển đổi mượt mà trở lại `BROADCASTING`, tiếp tục phát tiếng dịch.

### Kịch Bản 4: Kịch Bản Paused Và Resume Phòng Họp
*   **Bước 1:** Phòng đang gặp hiện tượng trễ Whisper nặng, trạng thái đang là `SPEECH_DELAYED`.
*   **Bước 2:** Host nhận thấy cuộc họp cần tạm dừng để chuẩn bị tài liệu $\rightarrow$ Bấm nút Tạm dừng phòng họp $\rightarrow$ Phát sự kiện `room_pause` $\rightarrow$ Chuyển trạng thái sang `PAUSED`.
...
*   **Bước 4:** Host bấm nút Tiếp tục cuộc họp $\rightarrow$ Phát sự kiện `room_resume` $\rightarrow$ Trạng thái lập tức chuyển về `BROADCASTING`.
...
*   **Bước 1:** Phòng họp kết thúc $\rightarrow$ Trạng thái chuyển sang `SAVING_OUTPUTS`.
*   **Bước 2:** Trình tạo tài liệu hậu kỳ gọi dịch vụ gRPC `TranscriptService` để kết xuất tệp bản dịch chính thức nhưng gặp sự cố mạng, dịch vụ ngoài không phản hồi $\rightarrow$ Quá số lần thử lại tại chỗ (3 lần) $\rightarrow$ Chuyển trạng thái sang `SAVE_FAILED`.
