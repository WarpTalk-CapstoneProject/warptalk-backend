...
- [ ] T055 [Ops] Rebuild and smoke runtime images after T049-T052.
  ...
  - AssistantService local Release publish succeeds; a runtime smoke image rebuilt from that publish boots in compose and served the 2026-08-27 gateway E2E checks. AssistantService Dockerfile now copies `Directory.Build.props` and lock files before restore, ignores `.tmp/`, disables first-time workload/telemetry/audit noise, and uses BuildKit NuGet package/cache mounts plus `--ignore-failed-sources` locked restore to make retrying Docker builds less fragile. A direct SDK container restore with the host NuGet cache mounted succeeds, proving the project graph is restorable in Linux. A minimal Docker restore harness shows the unseeded-container path is not deadlocked but spends minutes downloading packages from NuGet (`Microsoft.EntityFrameworkCore.Design` index alone took ~67s) and timed out after 180s while still progressing. A container-only EF design-time exclusion was rejected because it conflicts with locked restore package references, so it was not kept. Re-running standard compose build with clean context stayed in restore for more than 10 minutes without a clean result; this task cannot be checked yet.
  - **2026-08-28: root cause is not restore contention.** Removed `--disable-parallel` (it guarded against a deadlock theory the earlier diagnosis had already disproven) and timed a clean `docker compose build assistant-service`: it ran 24.5 minutes and failed. The restore log shows `Grpc.Tools.2.60.0` truncating mid-download from `api.nuget.org` three separate times ("response ended prematurely, with at least N additional bytes expected"); NuGet's retries eventually moved past it with a partially-broken restore, and `dotnet publish --no-restore` then failed with `NETSDK1064` on a missing `Microsoft.CodeAnalysis.Analyzers` package. This is network-level truncation on a larger `.nupkg`, not an MSBuild/restore-graph problem — parallel vs. serial restore was never the actual variable. Not re-diagnosed further because this may be specific to the sandboxed network path of the tool that ran the build rather than a real defect in the Dockerfile; needs a re-run from an unsandboxed host/CI runner before concluding anything about the fix. Still open.
...
- T055's Docker build blocker is unresolved as of 2026-08-28: standard `docker compose build assistant-service` still doesn't finish cleanly. `--disable-parallel` was removed (2026-08-28) on the theory it was serializing restore for no reason; the timed re-run instead showed the real cause is network-level truncation on a larger NuGet package download (`Grpc.Tools.2.60.0`), not restore contention — see T055's note. Needs re-testing from an unsandboxed host/CI runner.
