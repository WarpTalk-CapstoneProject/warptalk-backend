...
public sealed class InvoiceOverdueSweeper : BackgroundService
{
    ...
    public const string LockResource = "billing:invoice-overdue";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                // SINGLE RUNNER. Reminders are already de-duplicated (SET NX) and suspensions guarded by xmin;
                // the lease stops concurrent replicas from racing into those guards every tick.
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var invoice in candidates)
        {
            ...
            var dedupeKey = string.Format(RedisConstants.Keys.InvoiceReminderDedupeTemplate, invoice.Id, kind);
            var shouldSend = await redis.StringSetAsync(dedupeKey, RedisConstants.Values.Sent, TimeSpan.FromDays(45), When.NotExists);
            if (!shouldSend)
                continue;
            ...
            await notificationClient.SendNotificationsAsync(
                new SendBillingNotificationsRequest(
                    ...
                    BillingMessageConstants.NotificationTitles.BillingInvoiceReminder,
                    ...
                    {
                        [BillingMessageConstants.NotificationMetadataKeys.InvoiceId] = invoice.Id.ToString(),
                        [BillingMessageConstants.NotificationMetadataKeys.InvoiceNumber] = invoice.InvoiceNumber,
                        [BillingMessageConstants.NotificationMetadataKeys.ReminderKind] = kind
                    }),
                cancellationToken);
        }
    }
