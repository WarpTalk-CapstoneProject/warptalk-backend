...
public class WorkspaceInvitationEmailComposer : IWorkspaceInvitationEmailComposer
{
        ...
        CancellationToken ct = default)
    {
        ...
        var htmlTemplate = await LoadTemplateHtmlAsync(appBaseUrl, ct);
    ...
    private async Task<string> LoadTemplateHtmlAsync(string appBaseUrl, CancellationToken ct)
    {
        try
        {
            // 1. Check local web template file path if running in same monorepo root
            var webTemplatePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", "..", "..", "..", "..", "warptalk-web", "public", "templates", "workspace-invitation-email.html");
            ...
            var localTemplatePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Templates", "workspace-invitation-email.html");
