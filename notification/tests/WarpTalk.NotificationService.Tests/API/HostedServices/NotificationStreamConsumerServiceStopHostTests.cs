...
/// whole NotificationService process down — the admin notification read APIs included.
///
/// Streams counterpart of
/// BillingRedisSubscriberServiceTests.StartAsync_WhenRedisSubscribeFails_DoesNotThrow.
/// </summary>
public sealed class NotificationStreamConsumerServiceStopHostTests
{
    [Fact]
    public async Task StartAsync_WhenConsumerGroupCreationFails_DoesNotThrow()
    {
        var logger = new Mock<ILogger<NotificationStreamConsumerService>>();
    ...
    /// <summary>
    /// Surviving startup is only half the contract: a worker that silently never created its
    /// group looks healthy while no admin notification is ever delivered.
    /// </summary>
