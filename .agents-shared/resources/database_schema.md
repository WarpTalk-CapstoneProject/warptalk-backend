# WarpTalk — Database Schema Design (Production-Grade)

> **Pattern**: Schema-per-service (PostgreSQL)  |  **UUID**: v7 (time-ordered via `pg_uuidv7`)
> **All 12 skill audit gaps applied** — indexes, FK strategies, constraints, partitioning
...
```sql
-- ============================================
-- AUTH SCHEMA — Users, RBAC, Workspaces, Audit
-- ============================================
...
-- Default: system_admin, workspace_admin, host, participant
...
CREATE TABLE auth.user_roles (
    ...
    workspace_id UUID,  -- NULL = global role
...
-- Create monthly partitions: audit_logs_2026_01, audit_logs_2026_02, ...
...
erDiagram
    ...
    workspaces ||--o{ user_roles : scoped_to
    workspaces ||--o{ workspace_invitations : has
```

---

## 2. `translation_room` Schema

```sql
-- =============================================
-- MEETING SCHEMA — Lifecycle, Participants, Audio
-- =============================================
CREATE SCHEMA IF NOT EXISTS translation_room;

CREATE TABLE translation_room.translation_rooms (
    id               UUID PRIMARY KEY DEFAULT uuid_generate_v7(),
    workspace_id     UUID NOT NULL,     -- FK via gRPC → auth.workspaces
    host_id          UUID NOT NULL,     -- FK via gRPC → auth.users
...
CREATE TABLE translation_room.translation_room_participants (
    ...
    user_id              UUID NOT NULL,        -- FK via gRPC → auth.users
...
CREATE TABLE translation_room.translation_room_summaries (
    ...
    UNIQUE (translation_room_id)  -- 1 summary per translation_room
);
...
CREATE TABLE translation_room.translation_room_feedback (
    ...
    created_at            TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (translation_room_id, user_id)  -- 1 feedback per user per translation_room
);

-- INDEXES: translation_room
...
CREATE INDEX idx_translation_rooms_workspace_scheduled ON translation_room.translation_rooms(workspace_id, scheduled_at DESC);
...
```mermaid
erDiagram
    translation_rooms ||--o{ translation_room_participants : has
    translation_rooms ||--o{ translation_room_audio_routes : routes
    translation_rooms ||--o{ translation_room_recordings : produces
    translation_rooms ||--|{ translation_room_summaries : summarized_by
    translation_rooms ||--o{ translation_room_feedback : reviewed_in
    translation_room_participants ||--o{ translation_room_audio_routes : source
    translation_room_participants ||--o{ translation_room_audio_routes : target
```
...
```sql
-- ====================================================
-- TRANSCRIPT SCHEMA — Segments, Translations, Glossary
-- ====================================================
CREATE SCHEMA IF NOT EXISTS transcript;

CREATE TABLE transcript.transcripts (
    id              UUID PRIMARY KEY DEFAULT uuid_generate_v7(),
    translation_room_id      UUID NOT NULL,   -- FK via gRPC → translation_room.translation_rooms
...
CREATE TABLE transcript.transcript_translations (
    id               UUID PRIMARY KEY DEFAULT uuid_generate_v7(),
    segment_id       UUID NOT NULL REFERENCES transcript.transcript_segments(id) ON DELETE CASCADE,
    target_language  CHAR(5) NOT NULL,
...
-- Full-text search
CREATE INDEX idx_segments_fts ON transcript.transcript_segments
    USING GIN(to_tsvector('simple', original_text));
CREATE INDEX idx_glossary_fts ON transcript.glossary_terms
    USING GIN(to_tsvector('simple', source_term || ' ' || target_term));
```

```mermaid
erDiagram
    transcripts ||--o{ transcript_segments : contains
    transcript_segments ||--o{ transcript_translations : translated_to
    transcript_segments ||--o{ transcript_corrections : corrected_by
    transcripts ||--o{ transcript_exports : exported_as
...
```sql
-- =============================================
-- SUBSCRIPTION SCHEMA — Plans, Credits, Payments
-- =============================================
...
CREATE TABLE subscription.credit_transactions (
    ...
    description     VARCHAR(255),
    reference_id    UUID,
    reference_type  VARCHAR(30),   -- 'translation_room', 'admin_action', etc.
...
CREATE INDEX idx_subscriptions_active_expiring ON subscription.subscriptions(status, current_period_end)
    WHERE status = 'active';
...
erDiagram
    ...
    subscriptions ||--o{ credit_transactions : tracks
    subscriptions ||--o{ payments : pays
    subscriptions ||--o{ usage_records : uses
...
```sql
-- =============================================
-- NOTIFICATION SCHEMA — Multi-channel Delivery
-- =============================================
...
CREATE INDEX idx_notifications_user_unread ON notification.notifications(user_id, status)
...
## 6. Qdrant Vector DB Collections
...
| `transcript_embeddings` | 768 | `translation_room_id`, `segment_id`, `speaker_id`, `language` | AI Assistant context |

---

## 7. Cross-Service References (via gRPC)

```
translation_room.translation_rooms.host_id          → gRPC → auth.users.id
translation_room.translation_rooms.workspace_id     → gRPC → auth.workspaces.id
transcript.transcripts.translation_room_id → gRPC → translation_room.translation_rooms.id
subscription.subscriptions.user_id → gRPC → auth.users.id
notification.notifications.user_id → gRPC → auth.users.id
```
...
| Schema | Tables | Indexes | Partitioned | Key Features |
|--------|--------|---------|-------------|--------------|
