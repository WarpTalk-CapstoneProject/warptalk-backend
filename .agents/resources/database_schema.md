...
CREATE TABLE meeting.meeting_summaries (
    ...
    UNIQUE (meeting_id)  -- 1 summary per meeting
);
...
CREATE TABLE meeting.meeting_feedback (
    ...
    created_at            TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (meeting_id, user_id)  -- 1 feedback per user per meeting
);
...
CREATE INDEX idx_meetings_workspace_scheduled ON meeting.meetings(workspace_id, scheduled_at DESC);
...
erDiagram
    ...
    meetings ||--o{ meeting_audio_routes : routes
    meetings ||--o{ meeting_recordings : produces
    meetings ||--|{ meeting_summaries : summarized_by
    meetings ||--o{ meeting_feedback : reviewed_in
    meeting_participants ||--o{ meeting_audio_routes : source
    meeting_participants ||--o{ meeting_audio_routes : target
```
...
CREATE TABLE transcript.transcripts (
    id              UUID PRIMARY KEY DEFAULT uuid_generate_v7(),
    meeting_id      UUID NOT NULL,   -- FK via gRPC → meeting.meetings
...
CREATE TABLE subscription.credit_transactions (
    ...
    description     VARCHAR(255),
    reference_id    UUID,
    reference_type  VARCHAR(30),   -- 'meeting', 'admin_action', etc.
...
transcript.transcripts.meeting_id → gRPC → meeting.meetings.id
...
| `meeting` | **6** | 11 | — | Audio routing, feedback, CHECK |
